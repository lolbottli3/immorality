...
void C_ConfigManager::setup()
{
	...
	setup_item(&g_cfg.ragebot.noonshot, false, crypt_str("Ragebot.noonshot"));
	setup_item(&g_cfg.ragebot.onshotresolver, false, crypt_str("Ragebot.onshotresolver"));
	...
	setup_item(&g_cfg.ragebot.noteleport, false, crypt_str("Ragebot.slow_teleport"));
	...
	for (auto i = 0; i < 8; i++)
	{
		...
		setup_item(&g_cfg.ragebot.weapon[i].double_tap_shift_value, 13, crypt_str("Ragebot.double_tap_shift_value"));
	}

	setup_item(&g_cfg.antiaim.enable, false, crypt_str("Antiaim.enable"));
	setup_item(&g_cfg.antiaim.antibrute, false, crypt_str("Antiaim.antibrute"));

	setup_item(&g_cfg.antiaim.antiaim_type, 0, crypt_str("Antiaim.antiaim_type"));
	setup_item(&g_cfg.antiaim.hide_shots, false, crypt_str("Antiaim.hide_shots"));
	setup_item(&g_cfg.antiaim.hide_shots_key, key_bind(TOGGLE), crypt_str("Antiaim.hide_shots_key"));
	setup_item(&g_cfg.antiaim.hide_shots_shift_value, 7, crypt_str("Antiaim.hide_shots_shift_value"));
	...
	setup_item(&g_cfg.esp.Menucolor, Color(34, 125, 179, 255), ("Esp.Menucolor"));
	setup_item(&g_cfg.esp.textcolor, Color(255, 255, 255, 255), ("Esp.textcolor"));
	...
	setup_item(&g_cfg.esp.keybinds, false, crypt_str("Esp.keybinds"));
	...
	setup_item(&g_cfg.esp.new_scope_color, Color(255, 255, 255, 255), crypt_str("Esp.new_scope_color"));
	setup_item(&g_cfg.esp.penetration_reticle, false, crypt_str("Esp.penetration_reticle"));
	setup_item(&g_cfg.esp.new_scope, false, crypt_str("Esp.new_scope"));
	...
	setup_item(&g_cfg.esp.keybindposx, 10, crypt_str("Esp.keybindposx"));
	setup_item(&g_cfg.esp.keybindposy, 10, crypt_str("Esp.keybindposy"));
	...
	setup_item(&g_cfg.misc.killvoice, false, crypt_str("Misc.killvoice"));
	...
	setup_item(&g_cfg.misc.block_adv, false, ("misc.block_adv"));
	setup_item(&g_cfg.inventory.activate_inventory, false, ("inventory.activate_inventory"));
	setup_item(&g_cfg.inventory.autorarity, false, ("inventory.autorarity"));
	...
	setup_item(&g_cfg.misc.autoaccept, false, ("misc.autoaccept"));
...
void C_ConfigManager::save(std::string config)
{
	...
	WritePrivateProfileStringA("InventoryChanger", "Count", std::to_string(_inv.inventory.itemCount).c_str(), zfile.c_str());
	...
	for (auto weapon : g_InventorySkins)
	{
		...
		WritePrivateProfileStringA("InventoryChanger", itemid.c_str(), std::to_string(weapon.second.wId).c_str(), zfile.c_str());
		WritePrivateProfileStringA("InventoryChanger", paintkitstr.c_str(), std::to_string(weapon.second.paintKit).c_str(), zfile.c_str());
		WritePrivateProfileStringA("InventoryChanger", wearkitstr.c_str(), std::to_string(weapon.second.wear).c_str(), zfile.c_str());
		WritePrivateProfileStringA("InventoryChanger", seedkitstr.c_str(), std::to_string(weapon.second.seed).c_str(), zfile.c_str());
		WritePrivateProfileStringA("InventoryChanger", inusetstr.c_str(), weapon.second.in_use_t ? "true" : "false", zfile.c_str());
		WritePrivateProfileStringA("InventoryChanger", inusectstr.c_str(), weapon.second.in_use_ct ? "true" : "false", zfile.c_str());
		...
		WritePrivateProfileStringA("InventoryChanger", st1.c_str(), std::to_string(weapon.second.sicker[0]).c_str(), zfile.c_str());
		WritePrivateProfileStringA("InventoryChanger", st2.c_str(), std::to_string(weapon.second.sicker[1]).c_str(), zfile.c_str());
		WritePrivateProfileStringA("InventoryChanger", st3.c_str(), std::to_string(weapon.second.sicker[2]).c_str(), zfile.c_str());
		WritePrivateProfileStringA("InventoryChanger", st4.c_str(), std::to_string(weapon.second.sicker[3]).c_str(), zfile.c_str());
	...
	WritePrivateProfileStringA("MedalChanger", "Count", std::to_string(g_MedalSkins.size()).c_str(), zfile.c_str());
	i = 0;
	for (auto weapon : g_MedalSkins)
	{
		...
		std::string paintkitstr = "medal" + std::to_string(i) + "_paintkit";
		std::string equip = "medal" + std::to_string(i) + "_equip";
		WritePrivateProfileStringA("MedalChanger", paintkitstr.c_str(), std::to_string(weapon.second.paintKit).c_str(), zfile.c_str());
		WritePrivateProfileStringA("MedalChanger", equip.c_str(), std::to_string(weapon.second.equip_state).c_str(), zfile.c_str());
...
void C_ConfigManager::load(std::string config, bool load_script_items)
{
	...
	char value_l[32] = { '\0' };
	GetPrivateProfileStringA("InventoryChanger", "Count", "", value_l, 32, zfile.c_str());
	...
	for (int i = 0; i < _inv.inventory.itemCount; i++)
	{
		...
		GetPrivateProfileStringA("InventoryChanger", itemid.c_str(), "", value_l, 32, zfile.c_str());
		skinInfo.wId = atoi(value_l);
		GetPrivateProfileStringA("InventoryChanger", paintkitstr.c_str(), "", value_l, 32, zfile.c_str());
		skinInfo.paintKit = atoi(value_l);
		GetPrivateProfileStringA("InventoryChanger", wearkitstr.c_str(), "", value_l, 32, zfile.c_str());
		skinInfo.wear = atof(value_l);
		GetPrivateProfileStringA("InventoryChanger", seedkitstr.c_str(), "", value_l, 32, zfile.c_str());
		skinInfo.seed = atoi(value_l);
		GetPrivateProfileStringA("InventoryChanger", inusetstr.c_str(), "", value_l, 32, zfile.c_str());
		skinInfo.in_use_t = !strcmp(value_l, "true");
		GetPrivateProfileStringA("InventoryChanger", inusectstr.c_str(), "", value_l, 32, zfile.c_str());
		skinInfo.in_use_ct = !strcmp(value_l, "true");
		GetPrivateProfileStringA("InventoryChanger", qual.c_str(), "", value_l, 32, zfile.c_str());
		skinInfo.quality = atoi(value_l);
		GetPrivateProfileStringA("InventoryChanger", name.c_str(), "", value_l, 32, zfile.c_str());
		skinInfo.name = value_l;
		GetPrivateProfileStringA("InventoryChanger", stattrak.c_str(), "", value_l, 32, zfile.c_str());
		skinInfo.stattrak = atoi(value_l);


		GetPrivateProfileStringA("InventoryChanger", st1.c_str(), "", value_l, 32, zfile.c_str());
		skinInfo.sicker[0] = atoi(value_l);
		GetPrivateProfileStringA("InventoryChanger", st2.c_str(), "", value_l, 32, zfile.c_str());
		skinInfo.sicker[1] = atoi(value_l);
		GetPrivateProfileStringA("InventoryChanger", st3.c_str(), "", value_l, 32, zfile.c_str());
		skinInfo.sicker[2] = atoi(value_l);
		GetPrivateProfileStringA("InventoryChanger", st4.c_str(), "", value_l, 32, zfile.c_str());
	...
	g_MedalSkins.clear();
	char zvalue_l[32] = { '\0' };
	GetPrivateProfileStringA("MedalChanger", "Count", "", zvalue_l, 32, zfile.c_str());
	//atoi(zvalue_l);
	for (int i = 0; i < atoi(zvalue_l); i++)
	{
		...
		GetPrivateProfileStringA("MedalChanger", paintkitstr.c_str(), "", value_l, 32, zfile.c_str());
		int pk = atoi(value_l);
		GetPrivateProfileStringA("MedalChanger", equip.c_str(), "", value_l, 32, zfile.c_str());
		int eq = atoi(value_l);
...
void C_ConfigManager::config_files()
{
	...
	std::string path = folder + crypt_str("/*.im");
	WIN32_FIND_DATA fd;
