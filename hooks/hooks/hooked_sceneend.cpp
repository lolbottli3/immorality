...
void __fastcall hooks::hooked_sceneend(void* ecx, void* edx)
{
	...
	for (auto i = 1; i <= m_entitylist()->GetHighestEntityIndex(); i++)
	{
		...
		{
			m_materialsystem()->FindMaterial(crypt_str("Immorality_chams"), nullptr),
			m_materialsystem()->FindMaterial(crypt_str("Immorality_metallic"), nullptr),
			m_materialsystem()->FindMaterial(crypt_str("Immorality_flat"), nullptr),
			...
			m_materialsystem()->FindMaterial(crypt_str("Immorality_glow"), nullptr)
		};
