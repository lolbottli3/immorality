...
void ForceItemUpdate(weapon_t* zWeapon)
{
	...
	zWeapon->OnDataChanged(DATA_UPDATE_CREATED);
}
	...
	else if (!strcmp(event_name, crypt_str("player_death")))
	{
		...
		if (g_ctx.local()->is_alive() && attacker_id == m_engine()->GetLocalPlayer() && user_id != m_engine()->GetLocalPlayer())
		{
			...
			for (auto& wep : g_InventorySkins)
			{
				...
				if (wep.second.wId != g_ctx.local()->m_hActiveWeapon().Get()->m_iItemDefinitionIndex())
					continue;

				if (!wep.second.in_use_t && g_ctx.local()->m_iTeamNum() == 2)
					continue;

				if (!wep.second.in_use_ct && g_ctx.local()->m_iTeamNum() == 3)
					continue;

				wep.second.stattrak++;


				ForceItemUpdate(g_ctx.local()->m_hActiveWeapon().Get());
