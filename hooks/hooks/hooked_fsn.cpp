...
void __stdcall hooks::hooked_fsn(ClientFrameStage_t stage)
{
	...
	if (!g_cfg.inventory.activate_inventory)
		SkinChanger::run(stage);
	local_animations::get().run(stage);
	if (stage == FRAME_NET_UPDATE_POSTDATAUPDATE_START)
	{
		if (g_ctx.local() && g_cfg.inventory.activate_inventory)
				...
				if (!current_shot->hurt_player)
				{
					...
					if (current_shot->impact_hit_player)
					{
						...
						if (g_cfg.misc.events_to_log[EVENTLOG_HIT])
							eventlogs::get().addnew(crypt_str("Missed shot reason: failed resolve"),Color::Red);
					}
					else if (g_cfg.misc.events_to_log[EVENTLOG_HIT])
					{
						...
						if (current_shot->occlusion)
							eventlogs::get().addnew(crypt_str("Missed shot reason: occlusion"), Color::Red);
						else if (current_shot->shot_info.hitchance == 100)
							eventlogs::get().addnew(crypt_str("Missed shot reason: failed prediction"), Color::Red);
						else
							eventlogs::get().addnew(crypt_str("Missed shot reason: spread (inaccuracy)"), Color::Red);
					}
				}
			}
