...
void misc::watermark()
{
	...
	auto watermark = crypt_str("Immorality | ") + g_ctx.username /*+ crypt_str(" | ")*/ + g_ctx.globals.time;
	...
	if (m_engine()->IsInGame())
	{
		...
		if (nci)
		{
			...
			watermark = crypt_str("Immorality | ") + g_ctx.username /*+ crypt_str(" | ")*/ + server + crypt_str(" | ") + std::to_string(g_ctx.globals.ping) + crypt_str(" ms | ") + tickrate + crypt_str(" tick | ") + g_ctx.globals.time;
		}
	}
...
void misc::KillVoice(IGameEvent* event)
{
	...
	if (g_cfg.misc.killvoice)
	{
		...
		int health = m_engine()->GetPlayerForUserID(event->GetInt("health"));
...
void misc::hide_shots(CUserCmd* m_pcmd, bool should_work)
{
	...
	auto max_tickbase_shift = m_gamerules()->m_bIsValveDS() ? 6 : 16;
	auto tickbase_shift = min(g_cfg.antiaim.hide_shots_shift_value, max_tickbase_shift);
